...
pub fn generate_simulated_learners_for_strategy_3(
    lessons: &Vec<Lesson>,
    strategy: Strategy,
) -> (
    ...
    let mut base_simulated_learners_with_q_tables = HashMap::new();
    let mut collab_simulated_learners_with_q_tables = HashMap::new();
    ...
    for learner in learners {
        ...
        let learner_number = learner_id.chars().last().unwrap().to_digit(10).unwrap();

        if learner_number <= 3 {
            base_simulated_learners_with_q_tables.insert(learner_id, (learner, q_table.clone()));
        } else {
            collab_simulated_learners_with_q_tables.insert(learner_id, (learner, q_table.clone()));
        }
    }

    (
        (
            vec!["Learner 1", "Learner 2", "Learner 3"],
            base_simulated_learners_with_q_tables,
        ),
        (
            vec!["Learner 4", "Learner 5", "Learner 6"],
            collab_simulated_learners_with_q_tables,
        ),
    )
}
